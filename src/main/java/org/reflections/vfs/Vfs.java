 ...
 * 		return new HttpDir(url); // implement this type... (check out a naive
 * 									// implementation on VfsTest)
 * 	}
 * });
 *
 * Vfs.Dir dir = Vfs.fromURL(
 * 		new URL("http://mirrors.ibiblio.org/pub/mirrors/maven2/org/slf4j/slf4j-api/1.5.6/slf4j-api-1.5.6.jar"));
 * </pre>
 * <p>
 * use
 * {@link org.reflections.vfs.Vfs#findFiles(java.util.Collection, com.google.common.base.Predicate)}
...
public abstract class Vfs {
			...
			try {
				...
				if (Reflections.log != null) {
					Reflections.log.warn(
							"could not create Dir using " + type + " from url " + url.toExternalForm() + ". skipping.",
							e);
				}
			}
		}
	 ...
	 */
	public static Iterable<File> findFiles(final Collection<URL> inUrls, final String packagePrefix,
	 ...
	 * <p>
	 * jarUrl - creates a {@link org.reflections.vfs.ZipDir} over a jar url
	 * (contains ".jar!/" in it's name), using Java's {@link JarURLConnection}
	 * <p>
	 * directory - creates a {@link org.reflections.vfs.SystemDir} over a file
	 * system directory
	 * <p>
	...
	public static enum DefaultUrlTypes implements UrlType {
		...
		jarUrl {
			public boolean matches(URL url) {
				return "jar".equals(url.getProtocol()) || "zip".equals(url.getProtocol())
		...
		directory {
			public boolean matches(URL url) {
				return url.getProtocol().equals("file") && !hasJarFileInPath(url) && getFile(url).isDirectory();
			}
		...
		bundle {
			...
			public Dir createDir(URL url) throws Exception {
				return fromURL(
						(URL) ClasspathHelper.contextClassLoader().loadClass("org.eclipse.core.runtime.FileLocator")
